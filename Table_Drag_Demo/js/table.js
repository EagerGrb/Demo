...
function createSupensionTable(trElement, rect) {
    const htmlStrArr = []
...
class Table {
    ...
    initial() {
        if (!this.optional.cols || !Array.isArray(this.optional.cols) || this.optional.cols.length <= 0) {
            throw new Errow('init table col error.')
        }
        ...
        for (let i = 0; i < cols.length; i++) {
            ...
            const width = colItem.width ? colItem.width * 100 + '%' : defaultRatioPercent
            htmlStrArr.push(`<th width=${width} title=${colItem.title}>${colItem.title}</th>`)
        }
        htmlStrArr.push(`</tr>`)
        htmlStrArr.push(`</thead>`)
        htmlStrArr.push(`<tbody>`)
    ...
    updateTableContent(list) {
        this.optional.list = [...list]
        const { cols } = this.optional
        const htmlStrArr = []
        for (let i = 0; i < list.length; i++) {
            const rowItem = list[i]
            htmlStrArr.push(`<tr id='${rowItem.$id}' ${rowItem.$draggable ? 'draggable="true"' : ''}>`)
            for (let j = 0; j < cols.length; j++) {
                const cellItem = rowItem[cols[j].key]
                htmlStrArr.push(`<td title={${cellItem}}>${cellItem}</td>`)
            }
            htmlStrArr.push(`</tr>`)
        }
    ...
    updateTableStatus(showText) {
        ...
        htmlStrArr.push(`<tr title=${showText} colspan="${cols.length}>${showText}</td>`)
        htmlStrArr.push(`</tr>`)
        this.tbodyElement.innerHTML = htmlStrArr.join('\n')
    }
    rowDragStartHandler(e) {
        ...
        this.dragProfile.suspensionTable = createSupensionTable(copyTrElement, trElementBoundingClientRect)
